...
<LOG>;
<LOG>;
print LOG1 "MajorAllele\tChr\tStart\tEnd\tRef\tMut\tDepth\tReads\n";
while (<LOG>) {
	my @line = split/\t/;
	my $samtoolsoutput = `samtools mpileup -f /c4/home/limchen/spaceranger/refdata-gex-GRCh38-2020-A/fasta/genome.fa "$InputBam" -r chr"$line[1]":"$line[2]"-"$line[2]"`;
