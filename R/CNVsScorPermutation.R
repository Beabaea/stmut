...
caseCNVscore <- function(cdt0, armGens, arm, gainLoss){
  ...
  cdt1 <- rbind(cdt,newR)
  cdt1 <- cdt1[,order(cdt1[nrow(cdt1),],decreasing = TRUE)]

  scores <- cdt1[ncol(cdt1),]
  scor <- data.frame(t(scores))
  df <- data.frame(seq(dim(scor)[1]), rownames(scor), scor[,1])
  colnames(df) <- c("Rank", "Spot", "CNVScore")
  return(df)
}
...
fdrTumor <- function(a=0,b=1.82,c=4.4,data1,data2,data3,data4){
  ...
  ## patient6 rep2
  r2 <- data1 %>% filter(spot %in% data2$final)
  r22 <- data2 %>% filter(final %in% r2$spot)
  r22$barcode <- str_replace(r22$barcode,".1","-1")
  colnames(r2) <- c("final","Group")
  p6rep2 <- merge(r22, r2, by = "final")
  ...
  d2 <- data3 %>% filter(! Group == 0) %>% dplyr::select(barcode, Group)
  d3 <- rbind(p6rep2, d2)
